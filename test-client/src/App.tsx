...
type ConnectionState = {server: string, ns: string, counter: number, router: string}
...
const createConnection = (cid: number) => () => {
  ...
  socket.addEventListener('message', function (event) {
    ...
    if ( isServerMessage(msg)) {
      const sm = connMap.get(cid) || {server: msg.server, counter: 0, ns: msg.ns, router: msg.router};
      sm.counter = 0
    ...
    return (
      <tr>
        ...
        <td style = {
          {'borderBottom': '1px solid black',
          'minWidth': '6em'}
        } >{"ns-"+ idx % 16}</td>{servers.map(s => renderCell(idx, s))}
      </tr>
    )
  }
  const renderTable = () => {
